...
void GLQueueRunner::RunInitSteps(const std::vector<GLRInitStep> &steps, bool skipGLCalls) {
	...
	for (size_t i = 0; i < steps.size(); i++) {
		...
		case GLRInitStepType::CREATE_SHADER:
		{
			...
			if (!success) {
				...
				printf("Error in shader compilation for: %s", step.create_shader.shader->desc.c_str());
				printf("Info log: %s", infoLog.c_str());
	...
	if (allocatedTextures) {
		...
		if (err == GL_OUT_OF_MEMORY) {
			printf("GL ran out of GPU memory; switching to low memory mode");
			sawOutOfMemory_ = true;
		} else if (err != GL_NO_ERROR) {
			// We checked the err anyway, might as well log if there is one.
			printf("Got an error after init: %08x (%s)", err, GLEnumToString(err).c_str());
		}
	}
...
retry_depth:
	if (!fbo->z_stencil_) {
		printf("Creating %d x %d FBO using no depth", fbo->width, fbo->height);
		...
		if (gl_extensions.OES_packed_depth_stencil && (gl_extensions.OES_depth_texture || gl_extensions.GLES3)) {
			printf("Creating %d x %d FBO using DEPTH24_STENCIL8 texture", fbo->width, fbo->height);
			fbo->z_stencil_buffer = 0;
			fbo->stencil_buffer = 0;
			fbo->z_buffer = 0;
			...
			printf("Creating %d x %d FBO using DEPTH24_STENCIL8", fbo->width, fbo->height);
			// Standard method
			fbo->stencil_buffer = 0;
			fbo->z_buffer = 0;
			...
			printf("Creating %d x %d FBO using separate stencil", fbo->width, fbo->height);
			// TEGRA
			fbo->z_stencil_buffer = 0;
	...
	case GL_FRAMEBUFFER_COMPLETE:
		// printf("Framebuffer verified complete.");
		break;
