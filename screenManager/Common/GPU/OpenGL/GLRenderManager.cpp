...
void SCREEN_GLRenderManager::ThreadStart(SCREEN_Draw::SCREEN_DrawContext *draw) {
	...
	if (newInflightFrames_ != -1) {
		printf("Updating inflight frames to %d", newInflightFrames_);
	...
	do {
		...
		FrameData &frameData = frameData_[threadFrame_];
		{
			...
			while (!frameData.readyForRun && run_) {
				printf("PULL: Waiting for frame[%d].readyForRun", threadFrame_);
			...
			printf("PULL: Setting frame[%d].readyForRun = false", threadFrame_);
		...
		printf("PULL: Running frame %d", threadFrame_);
		if (firstFrame) {
			printf("Running first frame (%d)", threadFrame_);
			firstFrame = false;
		}
		Run(threadFrame_);
		printf("PULL: Finished frame %d", threadFrame_);
	} while (!nextFrame);
	return true;
}
...
void SCREEN_GLRenderManager::StopThread() {
	...
	if (run_) {
		...
		printf("GL submission thread paused. Frame=%d", curFrame_);
		...
		for (int i = 0; i < MAX_INFLIGHT_FRAMES; i++) {
			...
			while (!frameData.readyForFence) {
				printf("PUSH: Waiting for frame[%d].readyForFence = 1 (stop)", i);
				frameData.push_condVar.wait(lock);
			}
		}
	} else {
		printf("GL submission thread was already paused.");
	}
}
...
void SCREEN_GLRenderManager::BeginFrame() {
	...
	// Make sure the very last command buffer from the frame before the previous has been fully executed.
	{
		std::unique_lock<std::mutex> lock(frameData.push_mutex);
		while (!frameData.readyForFence) {
			printf("PUSH: Waiting for frame[%d].readyForFence = 1", curFrame);
			frameData.push_condVar.wait(lock);
		}
	...
	printf("PUSH: Fencing %d", curFrame);

	// glFenceSync(&frameData.fence...)

	// Must be after the fence - this performs deletes.
	printf("PUSH: BeginFrame %d", curFrame);
	if (!run_) {
		printf("BeginFrame while !run_!");
	}
...
void SCREEN_GLRenderManager::Finish() {
	...
	{
		std::unique_lock<std::mutex> lock(frameData.pull_mutex);
		printf("PUSH: Frame[%d].readyForRun = true, notifying pull", curFrame);
...
void SCREEN_GLRenderManager::Submit(int frame, bool triggerFence) {
	...
	if (triggerFence) {
		printf("PULL: Frame %d.readyForFence = true", frame);
...
void SCREEN_GLRenderManager::FlushSync() {
	...
	{
		std::unique_lock<std::mutex> lock(frameData.pull_mutex);
		printf("PUSH: Frame[%d].readyForRun = true (sync)", curFrame);
		frameData.initSteps = std::move(initSteps_);
		initSteps_.clear();
		frameData.steps = std::move(steps_);
		steps_.clear();
	...
	}
	{
		...
		while (!frameData.readyForFence) {
			printf("PUSH: Waiting for frame[%d].readyForFence = 1 (sync)", curFrame);
			frameData.push_condVar.wait(lock);
		}
		frameData.readyForFence = false;
		frameData.readyForSubmit = true;
...
void SCREEN_GLRenderManager::WaitUntilQueueIdle() {
	...
	for (int i = 0; i < MAX_INFLIGHT_FRAMES; i++) {
		...
		while (!frameData.readyForFence && frameData.readyForRun) {
			printf("PUSH: Waiting for frame[%d].readyForFence = 1 (wait idle)", i);
			frameData.push_condVar.wait(lock);
		}
	}
}
