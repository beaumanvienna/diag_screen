...
static void LogReadPixelsError(GLenum error) {
	...
	case GL_INVALID_ENUM:
		printf("glReadPixels: GL_INVALID_ENUM");
		break;
	case GL_INVALID_VALUE:
		printf("glReadPixels: GL_INVALID_VALUE");
		break;
	case GL_INVALID_OPERATION:
		printf("glReadPixels: GL_INVALID_OPERATION");
		break;
	case GL_INVALID_FRAMEBUFFER_OPERATION:
		printf("glReadPixels: GL_INVALID_FRAMEBUFFER_OPERATION");
		break;
	case GL_OUT_OF_MEMORY:
		printf("glReadPixels: GL_OUT_OF_MEMORY");
		break;
#ifndef USING_GLES2
	case GL_STACK_UNDERFLOW:
		printf("glReadPixels: GL_STACK_UNDERFLOW");
		break;
	case GL_STACK_OVERFLOW:
		printf("glReadPixels: GL_STACK_OVERFLOW");
		break;
#endif
	default:
		printf("glReadPixels: %08x", error);
		break;
	}
}
#endif
...
SCREEN_Pipeline *OpenGLContext::CreateGraphicsPipeline(const PipelineDesc &desc) {
	...
	if (!desc.depthStencil || !desc.blend || !desc.raster || !desc.inputLayout) {
		printf("Incomplete prim desciption");
		return nullptr;
	}
	...
	for (auto iter : desc.shaders) {
		if (iter) {
			...
			printf("ERROR: Tried to create graphics pipeline with a null shader module");
			delete pipeline;
			return nullptr;
		}
	}
	...
	if (pipeline->LinkShaders()) {
		...
		printf("Failed to create pipeline - shaders failed to link");
		delete pipeline;
		return nullptr;
	}
}
	...
	for (auto shaderModule : shaders) {
		if (shaderModule) {
			...
			if (shader) {
				...
				printf("LinkShaders: Bad shader module");
				return false;
			}
		} else {
			printf("LinkShaders: Bad shader in module");
			return false;
		}
	}
