...
void Buffer::Take(size_t length, std::string *dest) {
	if (length > data_.size()) {
		printf("Truncating length in Buffer::Take()");
		length = data_.size();
	}
	dest->resize(length);
	if (length > 0) {
...
void Buffer::Skip(size_t length) {
	if (length > data_.size()) {
		printf("Truncating length in Buffer::Skip()");
		length = data_.size();
	}
	data_.erase(data_.begin(), data_.begin() + length);
}
...
void Buffer::Printf(const char *fmt, ...) {
  ...
  if ((int)retval >= (int)sizeof(buffer)) {
    ...
    printf("Buffer::Printf truncated output");
  }
  if (retval < 0) {
    printf("Buffer::Printf failed");
  }
  va_end(vl);
...
bool Buffer::ReadAll(int fd, int hintSize) {
	...
	while (true) {
		...
		if (retval == 0) {
			...
			printf("Error reading from buffer: %i", retval);
			return false;
		}
